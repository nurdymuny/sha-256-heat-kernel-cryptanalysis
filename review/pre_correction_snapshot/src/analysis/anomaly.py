...
class AnomalyReport:
    ...
    diffusion_anisotropy: float       # Max/min diffusion rate ratio
    
    # Statistical tests
    eigenvalue_ks_statistic: float    # KS test vs null distribution
    eigenvalue_p_value: float         # p-value for KS test
    gap_ks_statistic: float           # KS test on spectral gaps
    gap_p_value: float
...
class AnomalyDetector:
    ...
    def analyze(
        ...
        obs_eigenvalues = observed.eigenvalues
        n_compare = min(len(obs_eigenvalues), len(null_dist.eigenvalue_mean))
        
        # KS test: are observed eigenvalues from same distribution as null?
        # Use z-scores
        z_scores = (obs_eigenvalues[:n_compare] - null_dist.eigenvalue_mean[:n_compare]) / (null_dist.eigenvalue_std[:n_compare] + 1e-10)
        
        # Test if z-scores are normally distributed (they should be under H0)
        eigenvalue_ks, eigenvalue_p = stats.kstest(z_scores, 'norm')
        
        # Spectral gap analysis
        obs_gaps = observed.spectral_gaps
        n_gap_compare = min(len(obs_gaps), len(null_dist.gap_mean))
        
        if n_gap_compare > 0:
            gap_z_scores = (obs_gaps[:n_gap_compare] - null_dist.gap_mean[:n_gap_compare]) / (null_dist.gap_std[:n_gap_compare] + 1e-10)
            gap_ks, gap_p = stats.kstest(gap_z_scores, 'norm')
            gap_variance = float(np.var(obs_gaps))
        else:
            gap_ks, gap_p = 0.0, 1.0
            gap_variance = 0.0
        
        # Compute metrics
        spectral_entropy = self._eigenvalue_entropy(obs_eigenvalues)
        
        # Diffusion anisotropy (from heat trace deviation)
        heat_trace_ratio = observed.heat_trace_values / (null_dist.heat_trace_mean + 1e-10)
        diffusion_anisotropy = float(np.max(heat_trace_ratio) / (np.min(heat_trace_ratio) + 1e-10))
        
        # Find anomalous modes (z-score > 3)
        anomalous_modes = list(np.where(np.abs(z_scores) > 3)[0])
        mode_descriptions = [
            f"Mode {i}: z={z_scores[i]:.2f} (λ={obs_eigenvalues[i]:.4f})"
            for i in anomalous_modes
        ]
        
        # Combined p-value (Fisher's method)
        if eigenvalue_p > 0 and gap_p > 0:
            chi2_stat = -2 * (np.log(eigenvalue_p) + np.log(gap_p))
            combined_p = 1 - stats.chi2.cdf(chi2_stat, df=4)
        else:
            combined_p = min(eigenvalue_p, gap_p)
        
        structure_detected = combined_p < self.alpha
        
        return AnomalyReport(
            spectral_entropy=spectral_entropy,
            spectral_gap_variance=gap_variance,
            diffusion_anisotropy=diffusion_anisotropy,
            eigenvalue_ks_statistic=float(eigenvalue_ks),
            eigenvalue_p_value=float(eigenvalue_p),
            gap_ks_statistic=float(gap_ks),
            gap_p_value=float(gap_p),
            structure_detected=structure_detected,
            significance_level=self.alpha,
            confidence=1 - combined_p,
            anomalous_modes=anomalous_modes,
            mode_descriptions=mode_descriptions,
            eigenvalue_histogram=obs_eigenvalues,
            null_histogram=null_dist.eigenvalue_mean,
            spectral_gap_series=obs_gaps
        )
