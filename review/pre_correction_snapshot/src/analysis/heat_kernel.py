...
class HeatKernelSolver:
    ...
    def spectral_curvature_estimate(
        ...
        HKS = self.heat_kernel_signature(result, np.array([t1, t2]))
        
        # From asymptotics: K(x,x,t) ≈ c(1 + Rt/6)
        # Ratio: K(t2)/K(t1) ≈ (1 + Rt2/6)/(1 + Rt1/6) ≈ 1 + R(t2-t1)/6
        ratio = HKS[:, 1] / (HKS[:, 0] + 1e-10)
        
        # Estimate R from ratio
        dt = t2 - t1
        curvature = 6 * (ratio - 1) / dt
