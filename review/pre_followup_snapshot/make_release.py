...
- `sha256_measurement.pdf`: complete paper with six measured figures.
- `sha256_reproducibility.zip`: source, evidence, tests, builder and manuscript.
- `arxiv_source.zip`: LaTeX submission sources and the six figure PDFs.
...
(ROOT/'README.md').write_text('''# Heat-Kernel Cryptanalysis of SHA-256

The current paper is **Heat-Kernel Cryptanalysis of SHA-256: A Geometric Study of State Evolution**, by Bee Rosa Davis.

The motivating question is whether SHA-256 has geometric regularities that could be exploited. The study uses graph heat kernels to examine sampled-state geometry and physical input interventions to follow perturbations through the computation. It develops the relationship between these descriptions and explains the research path toward the author's later dual-torus and double-cover work.
...
Six measured figures show heat profiles from full spectra, graph statistics, coordinate-resolved avalanche, held-out selection, cube transport and digest classification. See the paper for the full results, including the unresolved small detour contrasts and the limits of the tested selection procedures.
