...
def analyze_at_round(
    ...
    for i in range(n_pairs):
        ...
        # Actual input-message bit supplied by the generator metadata.
        flipped_bit = pair_bit_positions[i]
    ...
    if persistent_slow:
        ...
        for bit in persistent_slow[:20]:  # Show first 20
            if bit < 256:
                ...
                word = bit // 32
                bit_pos = bit % 32
                print(f"    Input bit {bit}: message W{bit//32}, MSB-first offset {bit%32}")
            else:
                # Message bit
        ...
        'results': [
            {
                ...
                'slow_bits': r.slow_bit_positions,
