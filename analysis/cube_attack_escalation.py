...
def run_heavy_cube(target_round, cube_bits):
    ...
    for i in range(limit):
        cube_mask = sum(1 << (511 - pos) for pos in cube_bits)
        msg_int = (0xCAFEBABE << 300) & ~cube_mask
        # Map bits from counter 'i' to message positions 'cube_bits'
        for bit_idx, cube_pos in enumerate(cube_bits):
            if (i >> bit_idx) & 1:
                msg_int |= (1 << (511 - cube_pos))
        
        # Add salt to ensure non-zero
        # Cube coordinates were cleared from the fixed base before assignment.
...
def check_subspace_rank(target_round, num_samples=500):
    ...
    from src.sha256.batch import gf2_rank
...
if __name__ == "__main__":
    ...
    is_zero_sum_17 = all(x == 0 for x in cube_result_17)
    
    # Analyze which words are zero_sum vs nonzero_sum
    word_names = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h']
    zero_sum_words = [word_names[i] for i, x in enumerate(cube_result_17) if x == 0]
    nonzero_sum_words = [word_names[i] for i, x in enumerate(cube_result_17) if x != 0]
    
    print(f"--> Round 17 Heavy Cube Result: {cube_result_17}")
    print(f"--> Broken words (sum=0): {zero_sum_words}")
    print(f"--> Secure words (sum≠0): {nonzero_sum_words}")
    print(f"--> Round 17 Status: {'🔴 FULLY ZERO SUM' if is_zero_sum_17 else '🟡 PARTIALLY ZERO SUM' if zero_sum_words else '✅ Secure'}")

    # 3. If 17 is partially zero_sum, try 18
    if zero_sum_words:
        cube_result_18 = run_heavy_cube(18, heavy_cube_bits)
        is_zero_sum_18 = all(x == 0 for x in cube_result_18)
        zero_sum_words_18 = [word_names[i] for i, x in enumerate(cube_result_18) if x == 0]
        nonzero_sum_words_18 = [word_names[i] for i, x in enumerate(cube_result_18) if x != 0]
        
        print(f"--> Round 18 Heavy Cube Result: {cube_result_18}")
        print(f"--> Broken words (sum=0): {zero_sum_words_18}")
        print(f"--> Secure words (sum≠0): {nonzero_sum_words_18}")
        print(f"--> Round 18 Status: {'🔴 FULLY ZERO SUM' if is_zero_sum_18 else '🟡 PARTIALLY ZERO SUM' if zero_sum_words_18 else '✅ Secure'}")
        
        # Try 19 if 18 still has zero_sum words
        if zero_sum_words_18:
            cube_result_19 = run_heavy_cube(19, heavy_cube_bits)
            is_zero_sum_19 = all(x == 0 for x in cube_result_19)
            zero_sum_words_19 = [word_names[i] for i, x in enumerate(cube_result_19) if x == 0]
            nonzero_sum_words_19 = [word_names[i] for i, x in enumerate(cube_result_19) if x != 0]
            
            print(f"--> Round 19 Heavy Cube Result: {cube_result_19}")
            print(f"--> Broken words (sum=0): {zero_sum_words_19}")
            print(f"--> Secure words (sum≠0): {nonzero_sum_words_19}")
            print(f"--> Round 19 Status: {'🔴 FULLY ZERO SUM' if is_zero_sum_19 else '🟡 PARTIALLY ZERO SUM' if zero_sum_words_19 else '✅ Secure'}")
    ...
    print("=" * 70)
    print("""
    Round 16: COMPLETELY ZERO SUM (all 8 registers, 7-bit cube)
    Round 17: PARTIALLY ZERO SUM (some registers, 16-bit cube)
