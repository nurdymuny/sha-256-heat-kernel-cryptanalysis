...
class DeepStructuralAnalyzer:
    ...
    def analyze_curvature_distribution(self, trajectories: List[SHA256Trajectory],
        ...
        """
        Compatibility method: return a truncated graph heat diagonal, not curvature.
        """
        ...
        for r in rounds:
            ...
            heat_diagonal = hks[:, 0]  # Truncated graph heat diagonal; no intrinsic curvature claim
            
            results[r] = {
                'mean_heat_diagonal': float(np.mean(heat_diagonal)),
                'std_heat_diagonal': float(np.std(heat_diagonal)),
                'min_heat_diagonal': float(np.min(heat_diagonal)),
                'max_heat_diagonal': float(np.max(heat_diagonal)),
                'heat_diagonal_range': float(np.max(heat_diagonal) - np.min(heat_diagonal))
            }
            
        self.results['heat_diagonal_analysis'] = results
        return results
    ...
    def analyze_spectral_clustering(self, trajectories: List[SHA256Trajectory],
        ...
        # Fit the same pipeline to independent random-bit data.
        ...
        results = {
            ...
            'silhouette_ratio': float(silhouette / random_silhouette) if random_silhouette > 0 else None,
    ...
    def analyze_geodesic_structure(self, trajectories: List[SHA256Trajectory]):
        ...
        from analysis.publication_study import detour
...
def run_full_analysis(n_samples=2000, n_pairs=1000, output_dir="visualizations"):
