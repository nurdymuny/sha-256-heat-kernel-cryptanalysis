...
- Paper: [`publication/release/sha256_measurement.pdf`](publication/release/sha256_measurement.pdf) (18 pages, 7 figures, 18 references)
...
**Proposition 1 (Laplacian invariants).** $L\sqrt{d}=0$ for $\sqrt d=D^{1/2}\mathbf 1$, and $L$ is unchanged under $W\mapsto cW$, $c>0$. The zero-mode residual is checked on every graph (below $10^{-10}$); invariance under rescaling is checked by a regression test.

**Proposition 2 (trace truncation).** With the eigenvalues in nondecreasing order, $Z(0)=n$, and for $Z_k(t)=\sum_{j<k}e^{-t\lambda_j}$, $0\le Z(t)-Z_k(t)\le(n-k)e^{-t\lambda_{k-1}}$. The paper uses full spectra, so the bound is a control on the earlier truncated computation (Section 5), not a step in the present one.
...
The receipts record $A_1(m,0)=2$ on all 256 bases of both partitions (a verifier check). The general mechanism, that adding $\pm2^k$ to a word changes bit $k$ and a carry chain above it, so that more significant positions change fewer bits early on, is stated in the paper as one source of the position dependence at round 1 (offsets 0, 1, 2 give mean responses 2.00, 2.48, 3.30; the full profile is not monotone) and is not claimed to yield exact values beyond Proposition 4.
...
**Proposition 6 (register transport).** Let $0\le j\le15$, let $S$ be a nonempty set of positions in $W_j$, and let $1\le r\le64$. As functions of the input, $b_r=a_{r-1}$; $c_r=a_{r-2}$ for $r\ge2$; $d_r=a_{r-3}$ for $r\ge3$; and likewise $f_r=e_{r-1}$, $g_r=e_{r-2}$, $h_r=e_{r-3}$, where an index $0$ denotes the constant initial-value word. Hence $\Delta_S d_r=\Delta_S a_{r-3}$ and $\Delta_S h_r=\Delta_S e_{r-3}$ for $r\ge3$, and $\Delta_S d_r=\Delta_S h_r=0$ for $r\le j+3$, $\Delta_S c_r=\Delta_S g_r=0$ for $r\le j+2$, $\Delta_S b_r=\Delta_S f_r=0$ for $r\le j+1$; for the smallest $r$ these are derivatives of constant words, which vanish because $S\ne\varnothing$.
...
- **Multiplicity.** Holm's step-down adjustment over the family of candidate-versus-one-half exact binomial tests (eight in the primary cube experiment, fourteen in the follow-up). At early rounds a one-half reference does not establish candidate specificity, since the average control rate is itself far from one half and individual sets range widely; the paper reports control ranges instead.
...
| Object | Result |
|---|---|
| Silhouette of the fitted 5-partition, $h=1$, SHA vs random (primary group, 20 pairs) | 0.896 vs 0.907; mean sorted cluster sizes (2.00, 2.05, 2.30, 3.65, 502.00) for SHA and (2.00, 2.05, 2.10, 3.55, 502.30) for random |
| Same, $h=8$ | 0.161 vs 0.164; mean sorted cluster sizes 81 to 121, individual partitions 62 to 141 |
...
| Entry-aligned avalanche | $W_0$ trajectory 3.8, 24.9, 55.6, 87.4, 115.6, 126.5, 127.9, 128.0 at $\delta=0,\dots,7$; the other message words follow closely similar values |
...
```
publication/
  sha256_measurement.tex          manuscript; every measured quantity enters through a generated macro (design constants are typed)
  numbers.tex, followup_numbers.tex, *_table.tex   generated from receipts
  derived.py                      every macro and table row, computed from the receipts
  build_paper.py, followup_tables.py               figures, tables, macros, pdflatex
  verify_evidence.py              20 checks: recomputes each receipt from its raw arrays, regenerates the macro files, checks source hashes
  make_release.py                 tests, verifier, archives, manifest, RESULTS.md
  figures/                        seven figures, PDF and PNG
  evidence/                       frozen receipts: protocol saved before execution, arrays, summaries, and source hashes where the runner records them
...
Runners refuse to overwrite existing receipts; rerun any experiment into a fresh directory (about a minute each). The intervention, search and cube arrays are integer arithmetic on seeded inputs and reproduce bit for bit under the stated implementation; graph statistics, detour ratios, classifier fits and bootstrap summaries involve floating-point operations and may differ in the last digits across platforms. Details, runtimes, a table mapping every number in the paper to its receipt and script, and three claims checkable by hand are in [`REVIEWER_GUIDE.md`](REVIEWER_GUIDE.md).
