...
# Complete-spectrum heat observables; no new sampling or model fitting.
times=np.geomspace(.01,100,241)
fig,axes=plt.subplots(2,2,figsize=(6.7,4.25),layout='constrained',sharex='col')
for col,h in enumerate(['1.0','8.0']):
    for cohort,data,style,cohort_color in [('Primary',g,'-',blue),('Confirmation',c,'--',orange)]:
        traces={label:np.array([np.exp(-times[:,None]*np.array(row['kernels'][h][label]['eigenvalues'])[None,:]).mean(axis=1) for row in data['records']]) for label in ['sha','random']}
        for label,color in [('sha',blue),('random',orange)]:
            axes[0,col].plot(times,traces[label].mean(axis=0),color=color,ls=style,lw=1.5,label=f'{label.upper()} / {cohort.lower()}')
        delta=traces['sha']-traces['random'];mean=delta.mean(axis=0);sd=delta.std(axis=0,ddof=1)
        axes[1,col].plot(times,mean,color=cohort_color,ls=style,lw=1.5,label=cohort)
        axes[1,col].fill_between(times,mean-sd,mean+sd,color=cohort_color,alpha=.13)
    axes[0,col].set_title(f'Kernel parameter h = {float(h):g}')
    axes[0,col].set_yscale('log');axes[0,col].set_ylim(.0015,1.1)
    axes[1,col].axhline(0,c='black',lw=.7,ls=':')
    axes[1,col].set_xlabel('Graph diffusion time t')
    axes[1,col].ticklabel_format(axis='y',style='sci',scilimits=(0,0),useMathText=True)
    for row in range(2):
        axes[row,col].set_xscale('log');axes[row,col].grid(alpha=.12)
axes[0,0].set_ylabel('Mean normalized heat trace')
axes[1,0].set_ylabel('Paired trace difference')
axes[0,0].legend(frameon=False,fontsize=6.5,loc='lower left')
axes[1,0].legend(frameon=False,fontsize=7,loc='lower left')
save(fig,'heat_profiles')
