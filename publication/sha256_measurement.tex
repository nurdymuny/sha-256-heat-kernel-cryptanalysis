...
\usepackage{lmodern,microtype,amsmath,amssymb,amsthm,booktabs,graphicx,array,float}
...
\begin{proposition}[Laplacian invariants]\label{prop:invariants}
With $d=W\mathbf 1$ entrywise positive, $L\sqrt d=0$ for $\sqrt d=D^{1/2}\mathbf 1$, and $L$ is unchanged when $W$ is replaced by $cW$ for any $c>0$.
\end{proposition}
\begin{proof}
$L\sqrt d=D^{-1/2}(D-W)D^{-1/2}D^{1/2}\mathbf 1=D^{-1/2}(D\mathbf 1-W\mathbf 1)=0$. Replacing $W$ by $cW$ replaces $D$ by $cD$ and leaves $D^{-1/2}WD^{-1/2}$ unchanged.
\end{proof}
The zero-mode residual is checked on every graph and is below $10^{-10}$; a regression test checks invariance under common weight rescaling. Nearest-neighbor construction, bandwidth and normalization are components of the estimator, not properties inferred from the data~\cite{luxburg}.
...
The release saves complete spectra and traces at $t=0.01,0.1,1,10$. In particular, $Z(0)=n$.
\begin{proposition}[Trace truncation]\label{prop:truncation}
With the eigenvalues in nondecreasing order and $1\le k\le n$, let $Z_k(t)=\sum_{j<k}e^{-t\lambda_j}$. Then $Z_k(0)=k$ and $0\leq Z(t)-Z_k(t)\leq(n-k)e^{-t\lambda_{k-1}}$ for all $t\ge0$.
\end{proposition}
\begin{proof}
$Z(t)-Z_k(t)=\sum_{j\ge k}e^{-t\lambda_j}$ has $n-k$ nonnegative terms, each at most $e^{-t\lambda_{k-1}}$ because $\lambda_j\ge\lambda_{k-1}$ for $j\ge k$.
\end{proof}
Full spectra avoid that truncation issue in this study; the bound quantifies what a truncated computation omits. Figure~\ref{fig:heat} evaluates the same saved spectra over a continuous plotting grid; it introduces no new sampled states or fitted models.

\begin{figure}[!htbp]\centering\includegraphics[width=\linewidth]{figures/heat_profiles.pdf}
...
\begin{proposition}[No curvature sign from the diagonal]\label{prop:diagonal}
For every vertex $i$, the map $t\mapsto K_t(i,i)$ is nonincreasing on $[0,\infty)$. Consequently $K_{t_2}(i,i)/K_{t_1}(i,i)\le1$ whenever $t_2>t_1$, for every graph and every $i$.
\end{proposition}
\begin{proof}
$\frac{d}{dt}K_t(i,i)=-\sum_j\lambda_j e^{-t\lambda_j}\phi_j(i)^2\leq0$, since every $\lambda_j\ge0$.
\end{proof}
A two-time diagonal ratio therefore does not establish the sign of an underlying manifold's scalar curvature, and the continuum expansion $K_t(x,x)\sim(4\pi t)^{-d/2}\bigl(1+R(x)t/6+\cdots\bigr)$ is not available on a finite graph without a specified limiting construction. Likewise, a Poincar\'e representation with prescribed negative curvature imposes a metric. When binary sign vectors are projected to a common radius, the resulting distance is a monotone function of Hamming distance. Neither construction alone demonstrates an intrinsic hyperbolic manifold.
...
\begin{figure}[!htbp]\centering\includegraphics[width=\linewidth]{figures/graph_controls.pdf}
...
The kernel parameter changes mean silhouette from approximately 0.90 to 0.16 in both populations. At $h=1$, the primary mean sorted cluster sizes are $\PrimaryOneSizesSHA$ for SHA and $\PrimaryOneSizesRandom$ for random data. The confirmation group has the same extreme imbalance, with mean largest clusters \ConfirmationLargestSHA{} and \ConfirmationLargestRandom{}. The score therefore describes a dominant cluster and a few very small groups in the fitted coordinates; it is not evidence for five substantive state populations or for a disconnected graph. At $h=8$, primary mean sorted sizes range from \PrimaryEightRangeSHA{} for SHA and \PrimaryEightRangeRandom{} for random data. The primary $h=8$ silhouette difference is slightly negative; that contrast does not repeat in the confirmation group. The spectral-gap intervals include zero in both groups at both kernel parameters.
...
\begin{figure}[!htbp]\centering\includegraphics[width=\linewidth]{figures/avalanche_entry.pdf}
...
Within W0, every coordinate enters at the same round. The early response nonetheless depends reproducibly on the position within that word (Table~\ref{tab:earlycoord}). Across rounds 1--5, the correlation of discovery and holdout position means is \EarlyCorrRange{}, and offset 0 is the smallest in both partitions. The correlation is still \EarlyCorrSix{} at round 6 and \EarlyCorrSeven{} at round 7; these samples do not define a universal disappearance threshold. This analysis was chosen after inspecting the frozen arrays and uses their existing independent partitions, not a third validation sample.
...
The round-1 endpoint has an exact explanation.
\begin{proposition}[Round-one response to the most significant bit]\label{prop:msb}
For every message $m$, $A_1(m,0)=2$.
\end{proposition}
\begin{proof}
In the first step $T_{1,1}=c+W_0$ with $c=h_0+\Sigma_1(e_0)+\operatorname{Ch}(e_0,f_0,g_0)+K_0$ constant, and $T_{2,1}$ is constant. Coordinate $0$ is bit $31$ of $W_0$, so $W_0(m\oplus e_0)=W_0(m)+2^{31}$ modulo $2^{32}$, both signs coinciding. Hence $a_1=T_{1,1}+T_{2,1}$ and $e_1=d_0+T_{1,1}$ each change by $2^{31}$, and adding $2^{31}$ toggles exactly bit $31$ of a 32-bit word. The other six working words of $X_1$ are components of the initial value. Exactly two bits change.
\end{proof}
Both saved partitions satisfy this identity on every base. Lower-position additions can also affect more significant bits through carries, one source of the position dependence seen at the first step; that dependence is not monotone in position. This explains the exceptional first-step MSB response; later rounds combine carries, rotations and Boolean mixing, and the data alone do not assign every later effect to one mechanism.
...
\begin{figure}[!htbp]\centering\includegraphics[width=\linewidth]{figures/holdout_search.pdf}
...
\begin{figure}[!htbp]\centering\includegraphics[width=\linewidth]{figures/early_structure.pdf}
...
\begin{lemma}[Higher-order derivatives~\cite{lai}]\label{lem:lai}
If every coordinate of $f$ has algebraic degree below $k$, then $\Delta_Sf\equiv0$ for every $S$ with $|S|=k$.
\end{lemma}
The converse fails: a zero sum on a particular cube, or at particular inputs, does not bound the degree. Cube attacks~\cite{dinur} and cube testers~\cite{testers} use related algebraic tools with different objectives. Our zero-rate measurements are cube tests on working-state projections; they do not perform key extraction or construct collisions or preimages.
...
\begin{table}[!htbp]\centering\small
\caption{Six-bit cube results. Differences and intervals are percentage points. The last column adjusts the eight candidate-versus-one-half binomial tests by Holm's method~\cite{holm}; it is not a test of the candidate-versus-control difference.}
...
At round 2, the candidate zero rate is \EarlyCubeTarget\%, versus \EarlyCubeControl\% averaged over the twenty uniform controls; the paired difference is \EarlyCubeDifference{} percentage points, with interval \EarlyCubeCI{}. The remaining thirteen uniform-control intervals contain zero (Table~\ref{tab:earlycube}). Three observations bound the interpretation. First, the uniform-control mean is itself far above one half at round 2, and the twenty uniform sets range from \UniformCubeMinTwo\% to \UniformCubeMaxTwo\%; a candidate-versus-one-half test at this round measures the round, not the candidate, so its adjusted $p$-value of \EarlyCubeP{} is not evidence of a candidate-specific effect. Second, every candidate position is more significant than the observed bit 16, and the round-2 zero rate of a uniform set increases with the number of its positions above bit 16 (correlation \UniformCubeAboveCorr{}). Against the forty significance-matched sets, evaluated on the same bases, the candidate rate remains above the matched-control mean of \MatchedCubeMeanTwo\%: the paired difference is \MatchedCubeDiffTwo{} points, with interval \MatchedCubeCITwo{}, a resolved contrast against that mean under the descriptive analysis. The candidate is not exceptional among individual matched sets: their rates range over \MatchedCubeRangeTwo\%, and \MatchedCubeAtOrAboveTwo{} of the forty lie at or above the candidate, with base-bootstrap 95\% bounds \MatchedCubeRankCITwo{} on that count. The count is a descriptive statistic, not an uncertainty interval, and the two statements are compatible. Third, the zero rate is a single-bit property: the complete word sum of register $a$ is zero for \WholeWordATwo\% of bases with the candidate and \WholeWordAControlTwo\% with uniform controls. Against the uniform family, rounds 3--24 do not resolve a candidate-minus-control difference. Against the matched family, \MatchedCubeLaterZeroCount{} of the \MatchedCubeLaterCount{} intervals at sampled rounds after round 2 contain zero; the exception is round \MatchedCubeLaterExceptions{}, \MatchedCubeSevenDiff{} points with interval \MatchedCubeSevenCI{}, an isolated unadjusted contrast among fourteen comparisons that requires confirmation on fresh bases before interpretation. The candidate-versus-one-half tests concern a different hypothesis and do not adjust these comparisons. Neither the control means nor the candidate rate proves a global algebraic-degree bound.

\begin{table}[!htbp]\centering\small
\caption{Fresh six-bit cube tests, 512 bases; zero rate of the six-fold derivative of bit 16 of register $a$, in percent. Uniform: twenty subsets of all of W0. Matched: forty subsets of offsets 0--14 on the same bases. Differences and intervals are percentage points. The sets column counts matched sets with zero rate at or above the candidate's; it is descriptive, and base-bootstrap bounds for round 2 are given in the text. Holm $p$ adjusts the fourteen candidate-versus-one-half tests; at round 2 it reflects the round, not the candidate.}\label{tab:earlycube}
...
These tests identify a round-2 bias in the single-bit six-fold derivative for almost every tested six-position set, strongest for sets drawn from the more significant half of W0; the candidate's rate exceeds the matched mean but lies within the spread of that family. The single-bit six-fold derivative has its own behavior: the whole-word three-bit entry control below cannot determine whether this different observable is balanced. Nor does a mean one-bit Hamming distance near 128 imply randomness of every higher-order response. The later-round rows remain measurements of those observables rather than consequences assumed from saturation.
...
\end{equation}
\begin{proposition}[Register transport]\label{prop:transport}
Let $0\le j\le15$, let $S$ be a nonempty set of positions in $W_j$, and let $1\le r\le64$. As functions of the input, $b_r=a_{r-1}$; $c_r=a_{r-2}$ for $r\ge2$; $d_r=a_{r-3}$ for $r\ge3$; and likewise $f_r=e_{r-1}$, $g_r=e_{r-2}$ for $r\ge2$, $h_r=e_{r-3}$ for $r\ge3$, where an index $0$ denotes the constant initial-value word. Hence $\Delta_S d_r=\Delta_S a_{r-3}$ and $\Delta_S h_r=\Delta_S e_{r-3}$ for $r\ge3$; and $\Delta_S d_r=\Delta_S h_r=0$ for $r\le j+3$, $\Delta_S c_r=\Delta_S g_r=0$ for $r\le j+2$, and $\Delta_S b_r=\Delta_S f_r=0$ for $r\le j+1$, where for the smallest $r$ these are derivatives of constant words.
\end{proposition}
\begin{proof}
The identities are the copy updates iterated. The cube sums are sums of the same functions over the same inputs. Finally $a_{r'}$ and $e_{r'}$ do not depend on $W_j$ for $r'\le j$, because $W_j$ first enters through $T_{1,j+1}$, and the $2^{|S|}$-fold XOR of a word that does not depend on $S$ vanishes because $S\neq\varnothing$.
\end{proof}
A trailing register can therefore retain a zero sum after a leading register changes, through exact transport alone. These identities give the observed pattern a precise meaning: the register view contains delayed copies of leading-word responses, while the nonlinear updates continually couple the two families.

\begin{figure}[!htbp]\centering\includegraphics[width=\linewidth]{figures/cube_controls.pdf}
...
The entry control displays the leading-to-trailing staircase predicted by Proposition~\ref{prop:transport}. Its exact zero fractions depend on the selected offsets and backgrounds, so we do not identify a universal round threshold. Total round count alone is an incomplete description of the experiment: both the entry time of the variables and the observation register are necessary to interpret a surviving cube sum.
...
\begin{figure}[!htbp]\centering\includegraphics[width=\linewidth]{figures/digest_classifiers.pdf}
...
The dynamical measurements resolve reproducible early structure within a word even when input entry is held fixed. The exact MSB endpoint (Proposition~\ref{prop:msb}) and the discovery--holdout coordinate agreement give that description an algebraic and an empirical anchor. The early equal-budget searches and round-2 cube tests add a boundary: against significance-matched controls the candidate's search advantage reverses, and its round-2 cube rate, though above the matched mean, lies within the spread of that class. Early advantage against uniform controls is therefore largely a property of position; the residual cube contrast is unresolved as candidate-specific, and neither comparison establishes a cryptanalytic improvement. The entry-aligned avalanche trajectories and cube-transport identities describe additional aspects of perturbation movement. They are complementary observables, not interchangeable definitions of diffusion. This combination provides a concrete starting point for investigating more detailed local geometry.
...
\bibitem{li2024asiacrypt} Y. Li, F. Liu, G. Wang, X. Dong and S. Sun. The first practical collision for 31-step SHA-256. \emph{ASIACRYPT 2024}, LNCS 15490, pp. 237--266. \url{https://doi.org/10.1007/978-981-96-0941-3_8}.
